...
struct SchedulerStateManager {
    ...
    // modify the other.
    // TODO(adams): this can be used to go from ActionInfoHashKey/Id (unique_qualifier) -> ActionInfo
            ...
            .remove(action_info)
        {
            Some(running_action) => {
                ...
                let send_result = if awaited_action.attempts >= self.max_job_retries {
                    ...
                    // TODO(adams): this logic and everything that leads to it needs to be Matching engine trait.
                    self.metrics.retry_action.inc();
                    Arc::make_mut(&mut awaited_action.current_state).stage = ActionStage::Queued;
                    let send_result = awaited_action
                        .notify_channel
        ...
        match action_state_results {
            Ok(mut stream) => {
                while let Some(action_state_result) = stream.next().await {
                    ...
                    // ActionStage is not copyable.
                    let copy_current_action_stage = awaited_action.current_state.stage.clone();
                    let unique_qualifier = awaited_action.action_info.unique_qualifier.clone();
                    // TODO(adams): do we bail on the first error?
                    let ret =
                        <SchedulerStateManager as MatchingEngineStateManager>::update_operation(
                            &mut self.scheduler_state_manager,
                            unique_qualifier,
                            maybe_worker_id,
                            Ok(copy_current_action_stage),
                            action_info.clone(),
                        )
                        .await;
                }
...
impl MatchingEngineActionStateResult {
    fn new(action_info: Arc<ActionInfo>) -> Self {
        Self { action_info }
    }
}
...
impl MatchingEngineStateManager for SchedulerStateManager {
    ...
    async fn update_operation(
        ...
        action_info_hash_key: ActionInfoHashKey,
        worker_id: Option<WorkerId>,
        action_stage: Result<ActionStage, Error>, // TODO(adams): Unused, action stage is directly set.
        action_info: Arc<ActionInfo>,
    ) -> Result<(), Error> {
        // TODO(adams); we need a method / function to go from ActionInfoHashKey/Id -> ActionInfo
        //   that can be done by
        // let Some(worker_id) = worker_id else {
        //     return Err("No worker ");
        // };

        let worker_id = worker_id.err_tip(|| "Expected worker id")?;
