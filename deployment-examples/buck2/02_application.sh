...
KUSTOMIZE_DIR=$(git rev-parse --show-toplevel)/deployment-examples/buck2
...
cd "$KUSTOMIZE_DIR" && kustomize edit set image \
    ...
    nativelink-worker-buck2=localhost:5001/nativelink-worker-buck2-toolchain:"$(\
        nix eval .#nativelink-worker-buck2-toolchain.imageTag --raw)"

kubectl apply -k "$KUSTOMIZE_DIR"

kubectl rollout status deploy/nativelink-cas
kubectl rollout status deploy/nativelink-scheduler
kubectl rollout status deploy/nativelink-worker-buck2
