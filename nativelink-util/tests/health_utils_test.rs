...
mod health_utils_tests {
    ...
    #[async_trait]
    pub trait MockComponent<'a>: Sync + Send + Unpin + HealthStatusIndicator<'a> {}

    #[derive(Debug)]
    struct MockComponentImpl;

    impl<'a> MockComponent<'a> for MockComponentImpl {
    ...
    impl<'a> HealthStatusIndicator<'a> for MockComponentImpl {
        ...
        //     Ok(HealthStatus::Ok) // (self.component_name(), "no error".into())
        // }
        async fn check_health(&self) -> Result<HealthStatus, Error> {
            Ok(HealthStatus::Ok)
        }
    ...
    async fn create_registery() -> Result<(), Error> {
        ...
        health_registery.register_collector(Arc::new(mock_component_impl));
    ...
    async fn create_nested() -> Result<(), Error> {
        ...
        // let mut health_registery = Box::new(HealthRegistry::<'_>::new("nativelink".into()));
        let mut health_registery = HealthRegistry::new("nativelink".into());
        ...
        nested1.register_collector(Arc::new(mock_component_impl2));

        let mock_component_impl3 = MockComponentImpl;

        let mut nested2 = health_registery.add_dependency("nested2".into());
        nested2.register_collector(Arc::new(mock_component_impl3));

        let mock_component_impl4 = MockComponentImpl;
        nested2.register_collector(Arc::new(mock_component_impl4));

        let nested5 = nested2.add_dependency("nested5".into());
        let mock_component_impl5 = MockComponentImpl;
        nested5.register_collector(Arc::new(mock_component_impl5));

        let mock_component_impl6 = MockComponentImpl;
        nested5.register_collector(Arc::new(mock_component_impl6));

        // let collections = health_registery.iter_collectors();
        // let p = std::pin::Pin::new(health_registery);

        let xx = health_registery.flatten_indicators();
